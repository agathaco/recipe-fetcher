...
1. **The app** (Next.js): the code that decides what HTML to show. It runs on a server and
   in your browser.
2. **The database** (Postgres, hosted on Neon): where the recipes are actually stored,
   permanently.
3. **The hosting** (Vercel): a computer on the internet that runs the app 24/7.
...
| **npm** | Node's "app store" plus installer. `npm install X` downloads library X into `node_modules/`. | How we get Next.js, Drizzle, and the rest. |
...
| **React** | The library for describing UI as components (reusable chunks of HTML and logic). | Next.js is built on it. |
...
| **Tailwind CSS** | Styling by putting utility classes in your HTML (`class="flex gap-4"`) instead of writing separate CSS files. | Boring and fast. The SPEC says don't think about styling. |
| **ESLint** | A nitpicker that flags sketchy code patterns. | Keeps the code tidy. |
| **Postgres** | A relational database. Data lives in tables with columns and rows, like linked spreadsheets. | The standard choice, and it pairs well with Vercel. |
| **Neon** | A company that runs Postgres for you in the cloud so you don't install anything. "Serverless" means it can sleep when unused and wake on demand. | Free, no setup, close to Vercel. |
| **Drizzle ORM** | A translator between your TypeScript code and SQL. You write `db.select().from(recipes)`, it writes `SELECT * FROM recipe`. | You still see real SQL, so you learn what's happening (unlike Prisma, which hides more). |
...
| **Git** | Tracks every change to your files, lets you undo and see history. | Standard, and Vercel deploys *from* git. |
...
Postgres is a **server program**, a process that runs all the time. It listens on a network
port (5432), owns a folder on disk where the data physically lives, and answers queries from
clients. Your app is a **client**: it opens a connection, sends SQL, gets rows back.
...
| **Managed instance** (AWS RDS, Google Cloud SQL, DigitalOcean) | The provider runs and patches it. But it's **on 24/7 whether you use it or not**. You pay per hour (roughly $15 to $50 a month for a small one) and still pick an instance size and worry about connection limits. |
| **Neon** (managed and serverless) | Provider runs it *and* the compute sleeps when idle, waking on the next query. Pay for actual usage. No instance size to choose. |
...
that opens about 10 connections once and reuses them.

But Vercel is **serverless**: every page request spins up a fresh, short-lived function.
Under load that's hundreds of functions, each opening its own connection, so you blow past
the limit, hit `too many connections`, and the site goes down. The functions are also too
short-lived to reuse a connection pool.

Neon is built for this:
- **Storage separated from compute.** The data lives in Neon's storage layer. The Postgres
  compute is stateless and can be killed or restarted freely, which is what lets it scale to
  zero and wake in a few hundred milliseconds.
...
- **Costs about $0.** The Free plan needs no credit card and doesn't do overage billing, and
  a personal recipe app stays far inside the limits. Scale-to-zero means you're not paying
  for an idle machine.
- **Solves the Vercel connection problem out of the box.** No PgBouncer to bolt on.
- **Still real Postgres.** Standard SQL and wire protocol. Run `pg_dump` and move elsewhere
  any time. Low lock-in at the data layer.
- **Region colocation.** Neon in Frankfurt, Vercel functions in `fra1`, so the app-to-DB hop
...
- **Steady, high traffic.** If the database is queried constantly, scale-to-zero buys you
  nothing, and a fixed-size instance is cheaper with no cold starts.
- **You need full transaction support everywhere.** The HTTP driver can't do transactions
  spanning multiple statements or `LISTEN/NOTIFY`, so you'd move to a normal connection.
...
- **Regulatory or data-residency rules** that dictate where and how the DB is run.
...
`npx` means "download and run this tool once." `create-next-app` is an official generator.
It asked no questions (we passed all the answers as flags: TypeScript yes, Tailwind yes,
App Router yes) and produced:

- `app/`: your pages live here. `app/page.tsx` is the homepage, and `app/layout.tsx` wraps
  every page (the `<html>` and `<body>` tags).
- `package.json`: the list of libraries the project uses, plus shortcut commands.
- config files (`next.config.ts`, `tsconfig.json`, and so on): leave them alone for now.
- `CLAUDE.md` and `AGENTS.md`: auto-generated notes for AI tools, warning that Next.js 16
  changed things versus older tutorials.
...
Made this folder a git repository.
...
(`-D` means "development only": tools you need while building but not when the site is live.)
...
- **`recipe`**: one row per recipe. Columns: `title`, `sourceUrl`, `sourceType`
  ...
  `steps`, `notes`, `wantToMake` (true/false), plus `createdAt` and `updatedAt` timestamps.
- **`tag`**: one row per tag name (for example "dessert"). Names must be unique.
- **`recipe_tag`**: the link table. A recipe can have many tags, and a tag applies to many
  recipes. This "many-to-many" needs its own table where each row is one pairing (recipe #5
  with tag "dessert"). This is the one data-model idea the SPEC wants you to learn.

`ingredients` and `steps` are just text on purpose, with no fancy structure. That's a
deliberate v1 limit.

**Files:** `db/index.ts` creates the `db` object your pages will import to run queries.
`drizzle.config.ts` tells `drizzle-kit` where the schema is and where to find the database
password.
...
**Migration** means a versioned change to the database structure. Each one is a file.
Running them in order takes any database from empty to current. The files are committed to
git so the same steps run on every environment.

**Verified:** queried Neon directly. `recipe`, `tag`, and `recipe_tag` all exist, empty but
there.

Before this: you created the Neon project (region: **eu-central-1 / Frankfurt**), copied the
**pooled** connection string, and pasted it into `.env.local` as `DATABASE_URL`.
...
**Hydration** means React re-checking, in the browser, the HTML the server already sent, and
attaching event handlers to it. If they don't match, it complains.

**Fix:** added `suppressHydrationWarning` to `<body>` in `app/layout.tsx`. This silences the
warning for that one tag only (not its contents), which is the standard fix because
extensions always mess with `<html>` and `<body>`.

### Step G: first commit, GitHub, Vercel
First commit made, then pushed to `github.com/agathaco/recipe-fetcher` over SSH (branch
`main`). Imported the repo into Vercel, set the `DATABASE_URL` environment variable, and
deployed. The empty app is live.

Still to do on the deploy: set the Vercel function region to `fra1` so it sits next to Neon
(takes effect on the next push anyway).

---

## 5. What's left to finish days 1 and 2

### Write the DECISIONS.md entries you owe
In your own words, using the template already in the file:
- **Postgres on Neon**: why hosted serverless Postgres over Supabase, Railway, or local
  Docker.
- **Drizzle over Prisma**: what "Prisma is more magic" actually costs you, and when you'd
  still pick Prisma.

Rule from the SPEC: if you can't write the entry confidently, you don't understand the
choice yet, so go read, then write it. Ask for an explanation, not for the text.
...
|---|---|---|
| 3-4 | List page and detail page | **Server Components.** The page queries the database directly on the server, and the HTML arrives already full of data, no loading spinner. |
...
| 10 | Auth and polish | A password check in `middleware.ts`, code that runs before every page. One user, so one shared password is enough. |
...
| `npm run build` | Do a production build, which catches type errors. |
...
| `npm run db:studio` | Open a browser GUI to view and edit the data. |
